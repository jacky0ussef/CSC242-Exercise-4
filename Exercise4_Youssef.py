...
Main Driver - Tests the LinkedList and LinkedListIterator classes within linkedlist.py.

Modified file: testlist.py

1) Given the following line of code found in the next and previous method of the
LinkedListIterator class: “if self._modCount != self._backingStore.getModCount():”,
identify the scenario when it will evaluate to true?

    - This will be true if the mod count is intentionally manipulated.

2) Explain what the following statement does in the AbstractCollection’s __str__ method
call: ", ".join(map(str, self)).

    - This method allows the object to be printed to the console, by converting the object into
    a string list representation of itself.

3) Create some diagrams similar to the LinkedList shown in Figure 1 that exhibits the
following code from the insert method (step by step—be sure to label theNode and newNode):
...
def testList(LystType):
    """Expects a list type as an argument and runs some tests
    on objects of that type.""" 
    print("\nTESTING THE LIST TYPE " + str(LystType))
    lyst = LystType(["Ken", 60, 1973])
